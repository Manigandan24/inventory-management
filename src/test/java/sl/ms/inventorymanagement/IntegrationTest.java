...
class IntegrationTest {
	...
	//@Test
	void getSpecificProductsTest() throws Exception {
		...
		MvcResult result = mockMvc
				.perform(MockMvcRequestBuilders.get("/products/1").headers(httpHeaders)
						.accept(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk()).andReturn();

		assertEquals(mapper.readValue(result.getResponse().getContentAsString(), Product.class), product);
	}
